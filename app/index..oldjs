...
module.exports = yeoman.generators.Base.extend({
	...
	prompting: function () {
		...
		this.prompt(prompts, function (props) {
			...
			this.webroot = props.repositoryName;
	...
	writing: {
		...
		app: function () {
			...
			async.series([
					...
					ioFileOperations('_configuration.php', this.webroot + '/configuration.php', true),
					ioFileOperations('_htaccess.txt', this.webroot + '/.htaccess', false),
		...
		clone: function() {
			...
			this.importCallBack = function(err, stdout, stderr) {
                            if (err)
                            {
                                throw err;
                            }
                            ...
                            rimraf(this.destinationRoot() + '/' + this.webroot + '/installation/', this.deleteInstallationDirectoryCallBack);
			}.bind(this);
			...
			this.writeCallBack = function(err) {
                            ...
                            cp.exec('mysql --user=' + this.db_user + ' --password=' + this.db_password + ' ' + this.db_database + ' < ' + 'joomla.sql', { cwd: __dirname + '/database' }, this.importCallBack);                                
			...
			this.replaceCallBack = function(err, data) {
                            ...
                            fs.writeFile('./' + this.webroot + '/database/joomla.sql', data, 'utf-8', this.writeCallBack);
			}.bind(this);
			...
			this.cloneCallBack = function(err, repo) {
                            ...
                            this.fs.copyTpl(
                                ...
                                this.destinationPath(this.webroot + '/configuration.php'),
                                params
                            );

                            this.fs.copy(
                                this.templatePath('_htaccess.txt'),
                                this.destinationPath(this.webroot + '/.htaccess')
                            );

                            fs.readFile('./' + this.webroot + '/installation/sql/mysql/joomla.sql', 'utf-8', this.replaceCallBack);
			}.bind(this);
			...
			else
			{
                            var download = new Download({ extract: true, strip: 1, mode: '755' })
                                ...
                                .dest(this.destinationPath(this.webroot))
                                .use(progress());
