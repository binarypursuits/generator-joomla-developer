...
<?php if (empty($this->items)) : ?>
	<p> <?php echo JText::_('COM_<%= uppercase %>_NO_WEBLINKS'); ?></p>
<?php else : ?>
...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
	...
	<fieldset class="filters btn-toolbar">
		<?php if ($this->params->get('filter_field') != 'hide') :?>
			<div class="btn-group">
				<label class="filter-search-lbl element-invisible" for="filter-search"><span class="label label-warning"><?php echo JText::_('JUNPUBLISHED'); ?></span><?php echo JText::_('COM_<%= uppercase %>_FILTER_LABEL') . '&#160;'; ?></label>
				<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('list.filter')); ?>" class="inputbox" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_<%= uppercase %>_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_<%= uppercase %>_FILTER_SEARCH_DESC'); ?>" />
			</div>
		<?php endif; ?>
				...
				<?php if (in_array($item->access, $this->user->getAuthorisedViewLevels())) : ?>
					...
					<div class="list-title">
						<?php if ($this->params->get('icons', 1) == 0) : ?>
							 <?php echo JText::_('COM_<%= uppercase %>_LINK'); ?>
						<?php elseif ($this->params->get('icons', 1) == 1) : ?>
							<?php if (!$this->params->get('link_icons')) : ?>
								<?php echo JHtml::_('image', 'system/weblink.png', JText::_('COM_<%= uppercase %>_LINK'), null, true); ?>
							<?php else: ?>
								<?php echo '<img src="' . $this->params->get('link_icons') . '" alt="' . JText::_('COM_<%= uppercase %>_LINK') . '" />'; ?>
							<?php endif; ?>
						<?php endif; ?>
						<?php
