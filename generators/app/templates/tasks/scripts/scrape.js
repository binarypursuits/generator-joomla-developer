...
function removeBlobStorageLinks(value, index, self)
{
    return value.indexOf('blob.core.windows.net') === -1;
}

function cleanLinkUrls(links)
{
    for (var i = 0; i < links.length; i++)
    {
        links[i] = links[i].replace('http://hilco', '');
    }
...
{
	return str.replace('.js', '-uncompressed.js');
}
...
casper.then(function() {
	...
	for (var i = 0; i < temp.length; i++)
	{
		if (temp[i].indexOf('.js') > -1)
		{
			...
			if (temp[i].indexOf('google-analytics.com') === -1 && temp[i].indexOf('maps.googleapis.com') === -1 && temp[i].indexOf('jquery.treeview.js') === -1 && temp[i].indexOf('static.formstack.com') === -1)
			{
				var source = temp[i];
	...
	{
		var href = temp[i];
		if (href.indexOf('.css') > -1 && href.indexOf('normalize.css') === -1 && href.indexOf('static.formstack.com') === -1)
		{
    ...
    /*
    temp = this.evaluate(getImages);
    temp = cleanLinkUrls(temp);
    temp = temp.filter(onlyUnique);
    images = temp.filter(removeBlobStorageLinks);
    */
});
